...
const DMVoiceAssistant = ({ dmScript }: DMVoiceAssistantProps) => {
  ...
  const startVoiceAssistant = async () => {
    if (!isApiKeySet || !apiKey) {
      toast({
        ...
        description: "Please enter your Vapi API key to start the voice assistant.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      vapiRef.current.on("call-end", () => {
        ...
        toast({
          ...
          description: "Your Dungeon Master voice assistant has ended the session.",
        });
      });
      ...
      vapiRef.current.on("error", (error) => {
        ...
        toast({
          title: "Error",
          description: "There was an error with the voice assistant. Please check your API key and try again.",
          variant: "destructive",
        });
        setIsLoading(false);
      });
      ...
      await vapiRef.current.start({
        model: {
          ...
          messages: [
            ...
            {
              role: "assistant",
              content: "Welcome, brave adventurers, to our Dungeons & Dragons session. I will be your Dungeon Master today. What would you like to do?"
            }
          ],
        },
        voice: {
      ...
      toast({
        ...
        description: "Failed to connect to the voice assistant service. Please check your API key and try again.",
        variant: "destructive",
      });
      setIsLoading(false);
    }
  ...
  return (
    <div className="mt-6 p-4 bg-parchment rounded-lg border border-brown-light">
      <h3 className="text-lg font-bold text-primary mb-4">Dungeon Master Voice Assistant</h3>
      
      {isLoading && <LoadingOverlay message="Connecting to DM voice assistant..." />}
      
      <div className="flex flex-col space-y-4">
        {/* API Key Input Section */}
        {!isApiKeySet ? (
          <div className="bg-white/50 p-4 rounded-lg">
            <h4 className="text-md font-semibold text-brown-dark mb-2">Enter Vapi API Key</h4>
            <p className="text-sm text-gray-600 mb-4">
              To use the voice assistant, you need a Vapi API key. 
              You can get this from the <a href="https://dashboard.vapi.ai/account" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">Vapi Dashboard</a>.
            </p>
            <div className="mb-2">
              <Label htmlFor="apiKey" className="text-sm font-medium text-brown-dark">
                API Key
              </Label>
              <Input 
                id="apiKey"
        ...
        <div className="mt-4 text-sm text-gray-600">
          <p>Speak to your DM assistant to progress through the campaign. The DM will narrate the story, describe environments, and roleplay NPCs based on the generated DM script.</p>
